...
const CURATE_SCHEMA = {
  ...
  properties: {
    top: {
      type: "array",
      description:
        "The best questions to take live, ordered best-first. At most 10. Deduplicate: if several people asked essentially the same thing, include it once (pick the best-phrased one) and mention in the note how many asked it.",
      items: {
        type: "object",
        properties: {
          ...
          note: {
            type: "string",
            description:
              "One short line for the moderator: why this question, and 'asked by N people' if it was a duplicate cluster.",
          },
        },
        required: ["id", "note"],
        additionalProperties: false,
...
export async function curateTop(
  questions: Question[]
): Promise<{ id: string; note: string }[]> {
  if (!process.env.ANTHROPIC_API_KEY || questions.length === 0) return [];
  const list = questions
    .map(
      (q) =>
        `id=${q.id} | ${q.name}${q.org ? ` (${q.org})` : ""}: ${q.question}`
    )
    .join("\n");
  const response = await client.messages.create({
    model: MODEL,
    ...
    output_config: {
      effort: "medium",
      format: { type: "json_schema", schema: CURATE_SCHEMA },
    },
    system: EVENT_CONTEXT,
    messages: [
      {
        role: "user",
        content: `Here are all audience questions submitted so far. Pick the top 10 (or fewer) for the moderator to take live, ordered best-first, deduplicating near-identical questions.\n\n${list}`,
      },
    ],
  });
  ...
  const text = response.content.find((b) => b.type === "text")?.text;
  if (!text) return [];
  const parsed = JSON.parse(text) as { top: { id: string; note: string }[] };
  const validIds = new Set(questions.map((q) => q.id));
  return parsed.top.filter((t) => validIds.has(t.id)).slice(0, 10);
}
